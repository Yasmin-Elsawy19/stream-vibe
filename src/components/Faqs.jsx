...
export default function Faqs() {
    ...
    const faqsData = [
        ...
        { id: "04", question: "How can I watch StreamVibe?", answer: "You can watch on your phone, tablet, or TV." },
        ...
        <section className="faqs-section">
            ...
            <div className="faq-grid">
                {faqsData.map((item, index) => {
                    const isActive = activeIndex === item.question;
                    return (
                        <div
                            key={`faq-${index}`}
                            className={`faq-item ${isActive ? "active" : ""}`}
                            onClick={() => setActiveIndex(isActive ? null : item.question)}
                        >
                            <div className="faq-content">
                                <span className="faq-number">{item.id}</span>
                                <div className="faq-text">
                                    <h4>{item.question}</h4>
                                    {isActive && <p className="faq-answer">{item.answer}</p>}
                                </div>
